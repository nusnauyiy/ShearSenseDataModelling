...
- Each video has resolution 60x60, meaning that eacah taxel is represented by 10x10 pixels. The video follows the naming format `{participant number}_{order}`, where a video with higher order represent an interaction happening at a later time during the data collection.
- An example frame of the version when a participant is pressing down on the sensor with fist is shown below. While we can see that the center taxels are activate, it is difficult to interpret the shear and pressure values directly.
...
- An example frame of the version when a participant is pressing down on the sensor with fist is shown below. We can see that the left-center taxels are activate. While there are no obvious x-shear in this case, the intensity for Cell 8 for a few taxels are lower, indicating a downward y-shear for these taxels.

...
def check_dataset():
    ...
    all_classes_names = os.listdir(config.DATA_DIR)
    ...
    for counter, random_index in enumerate(random_range, 1):
        ...
        video_files_names_list = os.listdir(f'{config.DATA_DIR}/{selected_class_Name}')

        selected_video_file_name = random.choice(video_files_names_list)

        video_reader = cv2.VideoCapture(f'{config.DATA_DIR}/{selected_class_Name}/{selected_video_file_name}')
...
def predict_on_live_video(model, video_file_path, output_file_path, window_size):
    ...
    video_writer = cv2.VideoWriter(output_file_path, cv2.VideoWriter_fourcc(*'mp4v'), 24,
